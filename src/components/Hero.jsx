...
const Hero = () => {
    return (
        <section className="bg-background-light dark:bg-background-dark font-sans min-h-screen flex items-center justify-center transition-colors duration-300 relative">
            ...
            >
                ...
                <p className="text-xl md:text-2xl text-secondary-text-light dark:text-gray-300 leading-relaxed mb-12 max-w-3xl mx-auto font-medium">
                    Frontend Web Developer <br className="hidden md:inline" />
                    With a strong background in building modern, scalable, and user-focused web applications.
                    solving real-world problems through clean code, structured architecture, and intuitive user interfaces..
                </p>

                <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 text-secondary-text-light dark:text-gray-300">
                    <SocialLink target='_blank' href="https://drive.google.com/file/d/1MZ4FxPv_eNrZHQW0-EzDdNt-uZlVFtM6/view?usp=drive_link" icon="far fa-file-alt" label="Resume" />
                    <SocialLink target='_blank' href="https://www.linkedin.com/in/md-mahmodul-hassen-khan-shadin/" icon="fab fa-linkedin" label="Linkedin" />
                    <SocialLink target='_blank' href="https://github.com/Shadhin282" icon="fab fa-github-square" label="github" />
                    <SocialLink href="#" icon="fab fa-figma" label="Figma" />
                </div>
            </motion.main>
        </section>
    );
...
import { FaFileAlt, FaLinkedin, FaGithub, FaFigma } from 'react-icons/fa';
...
const IconMapper = ({ iconClass }) => {
    ...
    if (iconClass.includes('figma')) return <FaFigma />;
    return null;
};
